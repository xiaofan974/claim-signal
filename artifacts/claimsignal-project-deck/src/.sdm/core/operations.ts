...
export function reorderRootElements(
  document: SlideDocument,
  elementIds: ReadonlyArray<string>,
  destinationIndex: number,
): SlideDocument {
  const movedIds = mutableRootSelection(document, [...elementIds]);
  if (movedIds === null) {
    return document;
  }
  const movedElements = document.elements.filter((element) =>
    movedIds.has(element.id),
  );
  const remainingElements = document.elements.filter(
    (element) => !movedIds.has(element.id),
  );
  const index = Math.max(
    0,
    Math.min(destinationIndex, remainingElements.length),
  );
  const next = [
    ...remainingElements.slice(0, index),
    ...movedElements,
    ...remainingElements.slice(index),
  ];

  return next.every(
    (element, elementIndex) => element === document.elements[elementIndex],
  )
    ? document
...
function randomIdSuffix(): string {
  let suffix = '';
  for (let index = 0; index < ID_SUFFIX_LENGTH; index += 1) {
    suffix += ID_SUFFIX_ALPHABET.charAt(
      Math.floor(Math.random() * ID_SUFFIX_ALPHABET.length),
    );
  }
...
export function nextId(document: SlideDocument, prefix: string): string {
  ...
  let candidate = `${prefix}-${randomIdSuffix()}`;
  while (used.has(candidate)) {
    candidate = `${prefix}-${randomIdSuffix()}`;
  }
...
function appendOffsetClones(
  document: SlideDocument,
  sources: ReadonlyArray<Element>,
): { document: SlideDocument; newIds: Array<string> } {
  const used = new Set<string>();
  collectIds(document.elements, used);
  const rename = (node: Element): void => {
    let candidate = `${node.id}-copy-${randomIdSuffix()}`;
    while (used.has(candidate)) {
      candidate = `${node.id}-copy-${randomIdSuffix()}`;
    }
    used.add(candidate);
    node.id = candidate;
    if (node.type === 'group') {
...
export function duplicateRootElements(
  ...
  return appendOffsetClones(
    document,
    document.elements.filter((element) => selected.has(element.id)),
  );
}

function visitElementAssetRefs(
  elements: ReadonlyArray<Element>,
  visit: (ref: { assetId: string }) => void,
): void {
  const visitPaint = (paint: Paint | undefined): void => {
    if (paint?.kind === 'image') {
      visit(paint);
    }
  };
...
export function collectElementAssetIds(
  ...
  visitElementAssetRefs(elements, (ref) => ids.add(ref.assetId));

  return [...ids];
}

/**
 * Appends clones of snapshot elements that need not exist in this document.
 * Referenced `assets` entries are adopted; an id the target maps to a
 * different file is remapped to a fresh id so both files keep rendering.
 */
export function pasteRootElements(
  document: SlideDocument,
  elements: ReadonlyArray<Element>,
  ...
  const adopted: Array<[string, Asset]> = [];
  const remap = new Map<string, string>();
  for (const assetId of collectElementAssetIds(elements)) {
    const asset = assets[assetId];
    if (asset === undefined) {
      continue;
    }
    if (!Object.hasOwn(existing, assetId)) {
      adopted.push([assetId, asset]);
      usedIds.add(assetId);
    } else if (!jsonEqual(existing[assetId], asset)) {
      let candidate = `${assetId}-copy-${randomIdSuffix()}`;
      while (usedIds.has(candidate)) {
        candidate = `${assetId}-copy-${randomIdSuffix()}`;
      }
      usedIds.add(candidate);
      adopted.push([candidate, asset]);
      remap.set(assetId, candidate);
    }
  }
  let sources = elements;
  if (remap.size > 0) {
    const remapped = elements.map((element) => structuredClone(element));
    visitElementAssetRefs(remapped, (ref) => {
      const next = remap.get(ref.assetId);
      if (next !== undefined) {
        ref.assetId = next;
      }
    });
  ...
  const pasted = appendOffsetClones(document, sources);
  if (adopted.length === 0) {
    return pasted;
  }
  ...
  return {
    ...
    document: {
      ...pasted.document,
      assets: { ...existing, ...Object.fromEntries(adopted) },
    },
  };
}

export function visualBounds(element: Element): Frame {
...
export function unionFrames(left: Frame, right: Frame): Frame {
