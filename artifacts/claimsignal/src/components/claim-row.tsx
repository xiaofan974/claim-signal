...
export function ClaimRow({ claim, index }: { claim: Claim; index: number }) {
  ...
  return <Link href={`/claims/${claim.claim_id}`} aria-label={`Open claim ${claim.claim_id} for ${claim.customer_name || 'unnamed customer'}`} className="group relative grid grid-cols-1 gap-4 border-b border-border/70 px-5 py-4 transition-colors hover:bg-[#f7f3e9] focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary sm:grid-cols-[1.1fr_1.2fr_.75fr_.7fr_auto] sm:items-center lg:px-6" data-testid={`row-claim-${claim.claim_id}`}>
    ...
    <ArrowUpRight size={17} aria-hidden="true" className="absolute right-5 text-muted-foreground/35 transition-colors transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-primary sm:static" />
  </Link>;
}