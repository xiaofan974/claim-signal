...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-[300px] flex-col border-r border-border bg-card text-foreground transition-transform duration-200 lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
        <div className="flex h-[82px] items-center justify-between border-b border-border px-7">
          <Link href="/" className="flex items-center gap-3" data-testid="link-brand">
            ...
            <span>
              ...
              <span className="mono block text-[9px] uppercase tracking-[.17em] text-muted-foreground">Operations cockpit</span>
            </span>
          </Link>
          <button type="button" onClick={() => setOpen(false)} className="rounded-md p-1 text-muted-foreground lg:hidden" data-testid="button-close-menu" aria-label="Close menu"><X size={18} /></button>
        </div>
        <div className="px-4 pt-8">
          <p className="mono mb-3 px-3 text-[10px] uppercase tracking-[.16em] text-muted-foreground">Workspace</p>
          <nav className="space-y-1">
            {navItems.map(({ href, label, icon: Icon }) => {
              const active = href === '/' ? location === '/' : location.startsWith(href);
              return <Link key={href} href={href} onClick={() => setOpen(false)} className={cn('group flex min-w-0 items-center gap-3 whitespace-nowrap rounded-lg px-3 py-2.5 text-[13px] font-medium transition-colors', active ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted/70 hover:text-foreground')} data-testid={`link-nav-${label.toLowerCase().replaceAll(' ', '-')}`}>
        ...
        <div className="mt-auto px-5 pb-6">
          <div className="rounded-xl border border-border bg-muted/45 p-4">
            <div className="mb-3 flex items-center gap-2"><span className="size-2 rounded-full bg-emerald-500 signal-pulse" /><span className="mono text-[10px] uppercase tracking-[.12em] text-muted-foreground">System status</span></div>
            <p className="text-[12px] leading-relaxed text-muted-foreground">Risk model and signal feed are operational.</p>
            <div className="mt-3 flex items-center gap-1.5 text-[10px] text-muted-foreground"><ShieldCheck size={13} /> Human review required for every action.</div>
          </div>
          <div className="mt-5 flex items-center gap-3 px-2">
            <span className="grid size-8 place-items-center rounded-full bg-primary/10 text-[11px] font-bold text-primary">JL</span>
            <div><p className="text-xs font-semibold">Jordan Lee</p><p className="text-[10px] text-muted-foreground">Claims operations</p></div>
          </div>
        </div>
      </aside>
