        ...
        <div className="grid gap-0 lg:grid-cols-[1.25fr_.75fr]">
            ...
            {claim.latest_customer_message && <blockquote className="mt-4 border-l-2 border-primary/30 pl-4 text-sm italic leading-relaxed text-muted-foreground">“{claim.latest_customer_message}”<footer className="mt-2 not-italic text-[11px] text-muted-foreground/70">Latest customer message</footer></blockquote>}
             {journeyLearning && <div className={`mt-4 rounded-lg border p-4 ${journeyLearning.tone === 'positive' ? 'border-emerald-200 bg-emerald-50/55' : 'border-amber-200 bg-amber-50/55'}`}><p className="mono text-[9px] font-medium uppercase tracking-[.14em] text-muted-foreground">Context check</p><p className="mt-1 text-sm font-semibold">{journeyLearning.title}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{journeyLearning.detail}</p></div>}
              <div className="mt-8"><SectionLabel>Claim timeline</SectionLabel><p className="mt-1 text-[11px] text-muted-foreground">Operational events recorded in Supabase</p><Timeline events={events} loading={eventsQuery.isLoading} /></div></>}</div>
           <div className="border-t border-border/80 bg-[#f8f5ed] p-5 sm:p-7 lg:border-l lg:border-t-0">
              <SectionLabel>AI interpretation</SectionLabel><p className="mt-1 text-[11px] text-muted-foreground">AI assessment based on the claim event record and current operational signals</p>{analysis ? <><div className="mt-4"><p className="text-[15px] leading-relaxed">{analysis.summary}</p></div>
...
function getJourneyLearning(claim: Claim, events: ClaimEvent[]) {
  const source = [claim.main_signal, claim.predicted_issue, claim.context_note, claim.latest_customer_message, claim.sentiment, ...events.flatMap((event) => [event.event_title, event.event_detail])].filter(Boolean).join(' ').toLowerCase();
  ...
  const negativeSentiment = (!explicitlyNoComplaint && /(complaint|complain|unhappy|frustrat|angry|escalat|negative)/.test(source)) || ['negative', 'deteriorating', 'frustrated'].includes((claim.sentiment ?? '').toLowerCase());
  ...
  const delta = (claim.risk_score ?? 0) - (claim.previous_risk_score ?? 0);
  if (delta < 0 && acceptedDelay) return { tone: 'positive' as const, title: 'Customer context lowers the apparent risk', detail: 'The recorded customer response explicitly accepts the delay, so the lower score reflects context rather than operational progress alone.' };
  if (operationalDelay && !negativeSentiment) return { tone: 'warning' as const, title: 'Operational delay is visible before negative sentiment', detail: 'The claim has delay-related signals even though the record does not show an explicit complaint. This is an early-warning condition, not evidence that the customer has complained.' };
  return null;
}
