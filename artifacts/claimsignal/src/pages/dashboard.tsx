    ...
    <div className="animate-rise-in">
      ...
      <section className="mt-9 rounded-xl border border-border bg-card shadow-sm">
        <div className="flex flex-col gap-4 border-b border-border/80 p-5 lg:flex-row lg:items-center lg:justify-between lg:px-6">
           <div><div className="flex items-center gap-2"><h2 className="font-semibold tracking-[-.02em]">Claims needing attention</h2><span className="rounded-full bg-muted px-2 py-0.5 mono text-[10px] text-muted-foreground" data-testid="queue-count">{claims.length}</span></div><p className="mt-1 text-xs text-muted-foreground">Ordered by risk, then most recent signal.</p><p className="mt-2 text-[11px] text-muted-foreground"><span className="font-semibold text-foreground">{isLoading ? '—' : stats.callbacks}</span> missed callbacks across these open claims</p></div>
